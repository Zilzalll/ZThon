...
async def mybot():
    ...
    await bot.send_message(
        BOTLOG_CHATID, "**- جـارِ إنشـاء البـوت التلقـائـي مـن بـوت فـاذر .. انتظـر قليـلاً**"
    )
    ...
    if zelzal.username:
        ...
        username = f"ZThon{(str(zelzal.id))[5:]}ubot"
    bf = "@BotFather"
    ...
    if not isdone.startswith("Good."):
        await bot.send_message(bf, f"مسـاعـد - {zelzal.first_name}")
        await asyncio.sleep(1)
        isdone = (await bot.get_messages(bf, limit=1))[0].text
        if not isdone.startswith("Good."):
            LOGS.info(
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = f"ZThon{(str(zelzal.id))[6:]}{str(ran)}ubot"
        await bot.send_message(bf, username)
        await asyncio.sleep(1)
        ...
        if nowdone.startswith("Done!"):
            ...
            await bot.send_message(bf, f"@{username}")
            await asyncio.sleep(1)
            await bot.send_file(bf, "zira/zilzal/logozed.jpg")
            await asyncio.sleep(3)
            await bot.send_message(bf, "/setabouttext")
            await asyncio.sleep(1)
            await bot.send_message(bf, f"@{username}")
            await asyncio.sleep(1)
            await bot.send_message(bf, f"- بـوت زدثــون المسـاعـد ♥️🦾 الخـاص بـ  {zelzal.first_name} ")
            await asyncio.sleep(3)
            ...
            LOGS.info(f"تم انشـاء البـوت المسـاعـد @{username} .. بنجـاح")
            await bot.send_message(
                BOTLOG_CHATID,
                "**- جـارِ اعـادة تشغيـل هيروكـو لضبـط المتغيـرات ...**",
            )
