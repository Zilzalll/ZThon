...
async def bot_help(event):
    await event.reply(
        f"""ᯓ 𝗭𝗧𝗵𝗼𝗻 𝗯𝗼𝘁 **- قائمــة اوامــر البـوت المسـاعـد 🤖♥️**
**⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆**
**📑 اولاً الاوامــر الإداريـة الخاصـه بالبــوت المسـاعــد الخـاص بـك :**
**ملاحـظــه :**
تعمـل هذه الأوامـر فقـط بخـاص البـوت المسـاعـد {botusername}

/uinfo <بالـرد ع رسـالة الشخـص>
• **الوصـف :**
لقد لاحظت أن الملصقات / الرموز التعبيرية المُعاد توجيهها لا تحتوي على علامة إعادة توجيه حتى تتمكن من تحديد المستخدم الذي أرسل تلك الرسائل بواسطة الامـر
• **ملاحظـه : **
يعمل لجميع الرسائل المعاد توجيهها. حتى بالنسبة للمستخدمين الذين يسمح لهم بإعادة توجيه الرسالة ، لا أحد

/ban <السبب> او /ban <المعرف/الايدي> <السبب>
• **الوصـف :**
الرد على رسالة المستخدم مع ذكر السبب حتى يتم إعلامه لأنك حظرت من البـوت ولن يتم إرسال رسائله إليك بعد ذلك
• **ملاحظـه :**
السبب لا بد منه. بدون سبب لن يعمل

/unban <السبب (اختياري)> او /unban <المعـرف/الايـدي>
• **الوصـف :**
الرد على رسالة المستخدم أو باضافـة اسم المستخدم / معرف المستخدم للامـر لإلغاء الحظر من البوت
• **ملاحظـه :**
لعـرض قائمـه المحظـورين في البـوت `{cmhd}المحظورين`.

/broadcast
• **الوصـف :**
قم بالرد على رسالة ليتم اذاعتها لكل مستخدم مشتـرك بالبـوت الخاص بك. للحصول على قائمـة المستخدمين استخدام `{cmhd}المشتركين`.
• **ملاحـظه :**
إذا قام المستخدم بحذف / حظر البـوت ، فسيتم إزالته من قاعدة البيانات الخاصة بك وسيتم مسحه من قائمة المشتـركـين

ٴ**⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆**

**📑 ثانيـاً اوامــر فارات تخصيص الكلايـش الخاصـه بالبــوت المسـاعــد الخـاص بـك :**
⪼ `{cmhd}اضف فار كليشة البوت`
