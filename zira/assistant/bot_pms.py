...
async def check_bot_started_users(user, event):
    ...
    if check is None:
        ...
        notification = f"**- سيـدي المطـور  🧑🏻‍💻**.\
                \n**- رابـط المستخـدم :** {_format.mentionuser(user.first_name , user.id)}\
...
async def bot_start(event):
    ...
    zz_username = user.username if user.username else "ZThon"
    ...
    if chat.id != Config.OWNER_ID:
        ...
        if customstrmsg is not None:
            ...
            start_msg = f"ٴ**𖣨 مـرحباً بـك عزيـزي  {mention} **\
                        \n\nٴ**𖣨 انـا البـوت الخـاص بـ {my_mention}.**\
                        \nٴ**𖣨 يمكنك التواصـل مـع مـالك البـوت هنـا 💌.**\
                        \nٴ**𖣨 فقـط قم بـ ارسـال رسـالتك وانتظـر الـرد 💡.**"
        buttons = [
            (
                Button.url("ᗪEᐯEᒪOᑭEᖇ 🧑🏻‍💻", f"https://t.me/{zz_username}")
            )
        ]
    else:
        start_msg = "ٴ**𖣨 مـرحبـاً بـك مـطـوري 🙈🖤..**\nٴ**𖣨 انا البـوت المسـاعـد زد الخـاص بـك 😼🦾**\nٴ**𖣨 يستطيـع اي شخص التواصل بك من خلالي**\nٴ**𖣨 لا تستغرب انني ايضاً بمثابة بوت تواصـل خاص بك✌️**\n\nٴ**𖣨 لرؤيـة اوامـري الخاصـه بـك اضغـط :  /help **"
        buttons = None
    try:
        if custompic:
