...
UPSTREAM_REPO_BRANCH = "zira"

REPO_REMOTE_NAME = "temponame"
IFFUCI_ACTIVE_BRANCH_NAME = "zira"
NO_HEROKU_APP_CFGD = "no heroku application found, but a key given? 😕 "
HEROKU_GIT_REF_SPEC = "HEAD:refs/heads/zira"
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    ...
    try:
        remote.push(refspec="HEAD:refs/heads/zira", force=True)
    ...
    try:
        remote.push("zira:main", force=True)
    except Exception as error:
...
async def upstream(event):
    ...
    try:
        ...
        origin.fetch()
        force_update = True
        repo.create_head("zira", origin.refs.zira)
        repo.heads.zira.set_tracking_branch(origin.refs.zira)
        repo.heads.zira.checkout(True)
