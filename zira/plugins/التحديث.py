    ...
    try:
        ...
        if conf is None:
            return await event.edit(
                f"Unfortunately, the directory {error} does not seem to be a git repository.\nBut we can fix that by force updating the userbot using `{cmdhd}تحديث الان`"
            )
...
async def upstream(event):
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    zzzz12 = await zzzz11.edit(f"ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗭𝗧𝗛𝗢𝗡 - تحـديثـات السـورس\n**•─────────────────•**\n\n**✾╎جـارِ . . تنصـيب التحـديث الجـذري ⎌**\n**✾╎يـرجى الانتظـار حتى تنتـهي العمليـة ⎋**\n**✾╎عادة ما يستغرق هـذا التحديث من 5 - 4 دقائـق 📟**")
    await deploy(zzzz12, repo, ups_rem, ac_br, txt)
